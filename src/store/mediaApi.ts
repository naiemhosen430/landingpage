...
export const mediaApi = api.injectEndpoints({
  endpoints: (builder) => ({
      ...
      transformResponse: (response: any) => {
        ...
        const normalize = (it: any) => ({
          publicId:
            it.publicId ??
            it.public_id ??
            it.public_id?.toString() ??
            it.public_id,
          url: it.url ?? it.url,
          secureUrl: it.secureUrl ?? it.secure_url ?? it.secure_url,
          format: it.format,
          width:
            typeof it.width === "number" ? it.width : Number(it.width) || 0,
          height:
            typeof it.height === "number" ? it.height : Number(it.height) || 0,
          bytes:
            typeof it.bytes === "number" ? it.bytes : Number(it.bytes) || 0,
          folder: it.folder,
    ...
    deleteMedia: builder.mutation<void, MediaDeletePayload>({
